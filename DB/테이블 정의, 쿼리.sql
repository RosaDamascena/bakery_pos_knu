...
| settlement_flag | enum('Y','N') | NO   |     | NULL              |                |
+-----------------+---------------+------+-----+-------------------+----------------+
...
INSERT INTO order_product (order_no, product_name, sale_volume) VALUES(1, '단팥빵',20), (1, '크림빵',20); 

SELECT * FROM order_info WHERE order_no = LAST_INSERT_ID();

SELECT * FROM order_product WHERE order_no = 1;
...
SELECT * FROM order_info WHERE order_no = 1;
